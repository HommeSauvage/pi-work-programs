...
| Retune a running program | `work_program({ action: "config", onExhausted, reviewProfile, maxParallel, workerModel, reviewerModel, reviewerResume, atlasEnabled, … })` — applies live and persists into plan.md front matter, so it survives sync and reload. `maxCycles` is deliberately **not** on this list: cycle budgets are operator-only (see below) |
| Retune one card | `work_program({ action: "config", card: "05", reviewProfile: "enhanced" })` — applies live and persists into that card's front matter (empty-string model clears the override) |
| Cycle budget (`maxCycles`) | **Operator-only.** Agents never raise or lower one — not to finish a stuck card, not to unblock the drive. `config` refuses `maxCycles` unless the operator explicitly asked for that exact change and the call carries `operatorApproved: true`; the change is then logged in `progress.md` as `(operator-authorized)`. The operator can always edit `plan.md` / the card's front matter by hand. Exhaustion is answered by `cycle_decision` (`accept` | `block`), never by a budget bump |
...
Gates are **discovered, never invented**. Whoever writes the plan reads the repository's own check commands — `package.json` scripts, CI workflows, `Makefile`/`justfile`, `turbo.json`/`nx.json`/`mise.toml`, `AGENTS.md`/`CONTRIBUTING.md` — and records the canonical one in the plan front matter (`gates.card`, inherited by every card), with per-card `gates:` overrides only where a card needs something narrower. `finalize_plan` warns when a program declares nothing, and every card's evidence then reads `gate: no gates configured` — legal, but its "gates green" is a claim, not a check. Evidence is measured, never assumed. The worker pastes exact command output and the commit SHA it was measured at into the card's `## Evidence` section. When gates are configured, the harness runs them itself and its result is authoritative; worker-pasted evidence is supplementary. A card is done only when its gate is green (or no gate applies) and its review has been triaged.

## Token economy

Measured on real programs, fresh workers spend ~20-25% of their input budget re-exploring the repo before their first edit, and each fresh review cycle re-derives the same understanding. Tokens are also a proxy, not the bill: a worker's context is ~43% tool results, ~26% tool-call arguments, ~29% thinking, ~1% instructions, while the dollars split roughly as **output 39% (half of it reasoning), cached input 39%, uncached input 22%** — and cached input costs ~$0.006/Mtok against $0.30/Mtok uncached and $1.20/Mtok output, so a transcript that is 98% cache reads is tracking the cheapest component. These mechanisms attack that:

- **Program atlas** (`atlas.md`): one scout explores the repo with the plan and every card in hand, then writes a capped orientation document — architecture, module map, conventions, integration points, negative knowledge, and 3-8 per-card file pointers. The first build gates worker dispatch; after each merge the scout is *resumed* with a surgical delta brief (the merged card ids, the commit log, the diffstat and the changed-file list), told to update in place — never to re-read the plan/cards it already holds and never to re-explore. The file is the source of truth (a replacement scout re-reads it instead of starting over); the session is only a warm cache. Briefs tell agents the atlas is orientation, not a boundary — verify before relying, explore beyond freely. An `atlas.md` you write yourself in the program dir is adopted as-is.
...
- **Run telemetry**: token usage is read from the child session transcript at run completion — input, cache reads/writes, output, reasoning, cost and its component split, turns — and keyed by session. Resumed runs replace their session's row, so card/program totals never double-count a resume chain. `status` shows program totals with the cache breakdown, each done card's harness evidence carries a `usage:` line such as `47.9M tok (cache 47.0M) · 340 turns · $0.82 (in 0.12 · out 0.20 · cache 0.21 · reason 82k)`, and the completion summary totals the program.
- **Adaptive resume**: continuations (fixes, re-reviews) resume the retained session while it is cheap to do so. Once a session's context peak passes `resumeMaxWindowPeak` (default 250k tokens) or `resumeMaxDepth` (default 3) consecutive resumes, the next pass is dispatched fresh with the atlas, the lane notes and a "reconstruct from Evidence + lane diff" brief. Measured: a resumed worker at 390k peak pays ~390k tokens on every remaining turn.
- **Fix-lane thinking**: reasoning is billed as output and is roughly half of it. `worker.fixThinking` / `worker.fixModel` (plan front matter `worker.fixThinking`, or per card `fixThinking:`) apply to **fresh fix dispatches only** — a resumed fix keeps the retained child's stored model and thinking, because pi-subagents revives the stored contract. Unset means the fix lane inherits the worker lane, so nothing changes until you ask for it.
- **Lane handoff notes**: every worker maintains a ≤60-line note at `<program>/.runtime/lanes/<card>.md` — decisions and why, invariants, dead ends, open threads, the files this lane owns. It is machine state (gitignored, outside the lane worktree), and any fresh continuation reads it before doing git archaeology. That is pi's own `/handoff` pattern: extract what matters for the next task instead of carrying the whole history.
- **Reviewer handoff notes**: the harness writes `<program>/.runtime/reviews/<card>.md` — one block per triaged review cycle (counts plus one bounded line per finding: approved, rejected, deferred, with the operator's note and the review path). Reviewers are read-only, so the harness is the writer; every review brief points at the note first, so a fresh reviewer does not re-raise a rejected or deferred finding without new evidence.
- **Gate discipline**: workers run fast scoped checks during implementation and the full quality gates **once, at the end** of the card — never in a mid-card loop (measured: 110+ bash calls per worker session before this). Reviewers work methodically — card first, diff second, **gates last** — running the gates themselves before writing findings (the shipped `work-program-reviewer` agent carries bash; the builtin pi-subagents `reviewer` does not, and old ledgers migrate to the shipped defaults). The harness also runs gates authoritatively at every transition. Per-card `gates:` front matter overrides the program gates; `gates: []` exempts a card.
...
pi compacts a session automatically once `context tokens > contextWindow - reserveTokens` (default reserve 16k, keeping ~20k of recent messages) and rebuilds the next request from the summary plus the kept tail. Our worker model declares a **1M** context window, so that threshold sits near 983k and never fires — the fresh-session threshold above was doing compaction's job by hand. To make pi's native compaction engage, give the lanes a bounded model alias (or a `compaction.modelOverrides` entry) in `~/.pi/agent/models.json` and point `workerModel` / `reviewerModel` / `atlas.model` at it:

- **worker/scout**: a ~180-200k window — long mechanical loops stay cheap.
- **reviewer**: a ~350-400k window — a review must not lose the diff mid-pass.
...
| `review.maxCycles` | `3` | Review/fix rounds before asking. Set once at plan time; operator-only afterwards (see the cycle-budget row above) |
| `review.onExhausted` | `ask` | `ask`, `accept`, or `block` |
| `worker.agent` / `review.agent` | `work-program-worker` / `work-program-reviewer` | Which subagents to spawn (plus optional `model`/`thinking`). Old ledgers migrate from the builtin `worker`/`reviewer` unless settings pin them explicitly |
| `worker.fixModel` / `worker.fixThinking` | unset (inherits the worker lane) | Fresh fix dispatches only (`fixModel`/`fixThinking` per card; a resumed fix keeps the retained contract) |
